...
public class CoverBehaviour : GenericBehaviour
{
    ...
    public float
        orientationSmooth =
            0.3f; // Default smooth factor to re-orientate player when turning corners or rounded covers.
    ...
    void Update()
    {
        ...
        if (takeCover)
        {
            ...
            if (isAiming && corner)
            {
                behaviourManager.GetCamScript.ToggleClampHorizontal(-horizontalClampAngle, horizontalClampAngle,
                    -coverWall.normal);
            }
            // Clamp off.
            else
    ...
    private bool HandleNavPath()
    {
        ...
        if (Vector3.Distance(transform.position, projectedCoverWall.point) > coverDist &&
            (possibleAction != CoverActions.CHANGE))
        {
            ...
            else
            {
                ...
                Vector3 signPosition = projectedCoverWall.point + (projectedCoverWall.normal * 0.01f) +
                                       (Vector3.up * 0.4f);
    ...
    void JumpOverMatchTarget()
    {
        ...
        {
            behaviourManager.GetAnim.MatchTarget(middlePoint, Quaternion.LookRotation(-coverWall.normal),
                AvatarTarget.Root,
    ...
    bool DrawPath(Vector3 destination)
    {
        ...
        if (!NavMesh.CalculatePath(transform.position + 2 * shift, ClosestNavMeshPoint(destination), NavMesh.AllAreas,
            path))
            return false;
    ...
    void DrawSign(GameObject sign, Vector3 destination, Vector3 normal, float xRot = 0f, int direction = 0,
        bool reScale = false)
    {
        if (sign != null)
        {
    ...
    private void JumpCoverManagement()
    {
        ...
        Vector3 castOrigin = transform.position - (maxJumpCoverDist + 0.1f + col.radius) * transform.forward +
                             standingHeight * Vector3.up;
        ...
        if (canJumpCover && currentAction == CoverActions.NONE)
        {
            ...
            jumpCoverEnd = transform.position - (AdjustCoverEnd(maxJumpCoverDist) + col.radius) * transform.forward +
                           standingHeight * Vector3.up;
    ...
    private float AdjustCoverEnd(float beginDist)
    {
        ...
        {
            beginDist -= 0.1f;
            stillJump = !Physics.Raycast(
                transform.position - (beginDist + col.radius) * transform.forward + standingHeight * Vector3.up,
    ...
    private void UpdateCoverParameters(Vector3 direction)
    {
        ...
        Debug.DrawRay(transform.position + (standingHeight * Vector3.up), -coverWall.normal,
            isCrouching ? Color.red : Color.green);
    ...
    private void CheckForCorners()
    {
        ...
        shift *= col.radius;
        Ray ray = new Ray(
            (castOriginHeight * Vector3.up) - (col.radius * 0.9f * transform.forward) + transform.position + shift,
            -coverWall.normal);
    ...
    private bool CanChangeCover(Ray ray, out RaycastHit hit)
    {
        ...
        bool canChangeCover =
            corner && Physics.Raycast(ray.origin + 5f * shift, ray.direction, out hit, 0.7f, coverMask);
    ...
    void Rotating(Vector3 direction)
    {
        ...
        Quaternion newRotation =
            Quaternion.Slerp(behaviourManager.GetRigidBody.rotation, targetRotation, orientationSmooth);
    ...
    private bool CanTakeCover()
    {
        return behaviourManager.IsGrounded() &&
               ...
               (behaviourManager.IsCurrentBehaviour(this.behaviourCode) ||
                behaviourManager.IsCurrentBehaviour(behaviourManager.GetDefaultBehaviour));
    }
