...
public class EntityTableImporter : AssetPostprocessor {
	...
	static void OnPostprocessAllAssets (string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths)
	{
		foreach (string asset in importedAssets) {
			...
			StatData data = (StatData)AssetDatabase.LoadAssetAtPath (exportPath, typeof(StatData));
			if (data == null) {
				data = ScriptableObject.CreateInstance<StatData> ();
			...
			using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read)) {
				...
				foreach(string sheetName in sheetNames) {
					...
					StatData.Sheet s = new StatData.Sheet ();
					s.name = sheetName;
					...
					for (int i=1; i <= sheet.LastRowNum; i++) {
						...
						StatData.Param p = new StatData.Param ();
					...
					cell = row.GetCell(4); p.ChangeCoverChance = (float)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(5); p.WeaponType = (cell == null ? "" : cell.StringCellValue);
					cell = row.GetCell(6); p.BulletDamage = (float)(cell == null ? 0 : cell.NumericCellValue);
